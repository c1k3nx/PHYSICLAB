...
export const CarnotSim: React.FC = () => {
  ...
  const draw = () => {
    ...
    let vol = 0.5; // 0 to 1
    if (p < 1) vol = 0.2 + 0.3 * p;         
    else if (p < 2) vol = 0.5 + 0.3 * (p-1);
    else if (p < 3) vol = 0.8 - 0.3 * (p-2);
    else vol = 0.5 - 0.3 * (p-3);           
    ...
    const r = Math.floor(59 + temp * (239-59)); 
    const b = Math.floor(246 - temp * (246-68)); 
    ...
    particlesRef.current.forEach(pt => {
        pt.x += pt.vx * particleSpeed; pt.y += pt.vy * particleSpeed;
    ...
    if (p < 1) { // Isothermal Expansion (Heat In)
        ...
        ctx.fillStyle = 'white'; ctx.fillText(`🔥 ${t('heat_in')}`, cx + cw/2 - 25, sourceY + 25);
        ...
        ctx.fillStyle = 'white'; ctx.fillText(`❄️ ${t('heat_out')}`, cx + cw/2 - 30, sourceY + 25);
        ...
        ctx.fillStyle = '#475569';
        ctx.fillText(t('insulated'), cx + cw/2 - 60, sourceY + 25);
    }
    
    // --- 3. P-V Diagram (Left Side) ---
    const gx = 50; const gy = h - 50; const gw = 250; const gh = 250;
    
    ctx.strokeStyle = 'white'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(gx, gy - gh); ctx.lineTo(gx, gy); ctx.lineTo(gx + gw, gy); ctx.stroke();
    ctx.fillText("P", gx - 20, gy - gh + 10); ctx.fillText("V", gx + gw + 10, gy);
    
    const getP = (v: number, t: number) => (t+0.5) / v; 
    
    ctx.strokeStyle = '#60a5fa'; ctx.lineWidth = 2; ctx.beginPath();
    const pts = [ {v: 0.2, p: getP(0.2, 1)}, {v: 0.5, p: getP(0.5, 1)}, {v: 0.8, p: getP(0.8, 0)}, {v: 0.5, p: getP(0.5, 0)} ];
    const sX = (v:number) => gx + v * gw; const sY = (p:number) => gy - (p/8) * gh;
    
    ctx.moveTo(sX(pts[0].v), sY(pts[0].p));
    ctx.quadraticCurveTo(sX(0.35), sY(pts[0].p), sX(pts[1].v), sY(pts[1].p)); 
    ctx.lineTo(sX(pts[2].v), sY(pts[2].p)); 
    ctx.quadraticCurveTo(sX(0.65), sY(pts[2].p), sX(pts[3].v), sY(pts[3].p)); 
    ctx.lineTo(sX(pts[0].v), sY(pts[0].p)); 
    ctx.stroke();
    
    const currentP = getP(vol, temp);
    ctx.fillStyle = '#facc15'; ctx.beginPath(); ctx.arc(sX(vol), sY(currentP), 6, 0, Math.PI*2); ctx.fill();
    
    const labels = ["iso_exp", "adia_exp", "iso_comp", "adia_comp"];
    const idx = Math.floor(p);
    ctx.fillStyle = 'white'; ctx.font = '16px font-bold sans-serif';
    ctx.fillText(t(labels[idx]), gx, gy - gh - 20);
  };

  const animate = () => {
      if (isRunning) { progressRef.current = (progressRef.current + 0.01 * speed) % 4; }
      draw();
      ...
      <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
           <div className="flex gap-4">
             ...
             >
                ...
                {isRunning ? t('pause_engine') : t('start_engine')}
             </button>
           </div>
           
           <div className="space-y-2">
               <label className="text-xs text-slate-400 font-bold uppercase">{t('sim_speed')}</label>
