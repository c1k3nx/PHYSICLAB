...
export const SpringSim: React.FC = () => {
  ...
  const drawSystem = () => {
      ...
      const targetScale = Math.min(1, (h - 150) / Math.max(currentY + 100, L0 + 100));
      ...
      ctx.fillRect(cx - 60, 0, 120, 20);
      ctx.strokeStyle = '#475569';
      ctx.lineWidth = 2;
      ctx.beginPath();
      for(let i=-50; i<60; i+=10) { ctx.moveTo(cx+i, 0); ctx.lineTo(cx+i-10, 20); }
      ctx.stroke();
      ...
      ctx.fillRect(cx - half, y, size, size/2);
      ...
      ctx.lineWidth = 2;
      ctx.beginPath(); 
      ctx.moveTo(cx - 120, eqY + half); 
      ctx.lineTo(cx + 120, eqY + half); 
      ctx.stroke();
      ctx.setLineDash([]);
  ...
  const drawGraph = () => {
      ...
      ctx.fillText(t('graph_pos_time'), 10, 20);
  };

  const animate = () => {
      if (isRunning) {
          for(let i=0; i<4; i++) updatePhysics(0.016/4);
      }
      drawSystem();
      drawGraph();
    ...
    <div className="flex flex-col h-full gap-4">
      <div className="flex flex-col md:flex-row gap-4 h-full min-h-0">
          ...
          <div className="w-full md:w-80 flex flex-col gap-4">
              <div className="bg-slate-900 rounded-xl border border-slate-700 p-2 h-48">
                   <canvas ref={graphRef} width={300} height={180} className="w-full h-full object-contain" />
              </div>
              
              <div className="flex-grow bg-slate-800 p-6 rounded-xl border border-slate-700 flex flex-col justify-end gap-6 shadow-xl">
                   <div className="space-y-2">
                        <label className="text-xs font-bold uppercase text-slate-400">{t('mass')} (m)</label>
                        <input type="range" min="1" max="10" step="0.5" value={mass} onChange={e => setMass(Number(e.target.value))} className="w-full accent-red-500"/>
                        <div className="text-right font-mono text-red-400">{mass} kg</div>
                   </div>
                   <div className="space-y-2">
                        <label className="text-xs font-bold uppercase text-slate-400">{t('stiffness')}</label>
                        <input type="range" min="20" max="200" step="10" value={k} onChange={e => setK(Number(e.target.value))} className="w-full accent-blue-500"/>
                        <div className="text-right font-mono text-blue-400">{k} N/m</div>
                   </div>
                   <div className="space-y-2">
                        <label className="text-xs font-bold uppercase text-slate-400">{t('damping')}</label>
                        <input type="range" min="0" max="1.0" step="0.05" value={damping} onChange={e => setDamping(Number(e.target.value))} className="w-full accent-purple-500"/>
                        <div className="text-right font-mono text-purple-400">{damping}</div>
                   </div>
                   
                   <div className="flex gap-2">
                     <button onClick={() => setIsRunning(!isRunning)} className={`flex-1 py-3 rounded-lg font-bold text-white shadow ${isRunning ? 'bg-amber-600' : 'bg-emerald-600'}`}>
                        {isRunning ? <Pause size={18} className="mx-auto"/> : <Play size={18} className="mx-auto"/>}
                     </button>
                     <button onClick={reset} className="p-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg">
                        <RotateCcw size={18}/>
                     </button>
                   </div>
              </div>
